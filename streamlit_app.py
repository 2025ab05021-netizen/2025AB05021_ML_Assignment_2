...
3. Then refresh this Streamlit app
    ...
    def show_predictions(models, scaler, metadata):
        ...
        @st.cache_data
        def prepare_data_for_prediction(_scaler, input_array):
            """Prepare input for prediction; leading underscore prevents Streamlit hashing the scaler."""
            try:
                if _scaler is not None:
                    return _scaler.transform(input_array)
            except Exception:
                pass
            return input_array
                ...
                if model is not None:
                    try:
                        ...
                        if name in ['logistic_regression', 'knn', 'naive_bayes'] and scaler is not None:
                            scaled_input = prepare_data_for_prediction(scaler, input_data)
    ...
    def show_performance_analysis(results_df):
        ...
        # Add AUC and MCC if available and numeric
        ...
        if 'MCC Score' in rank_df.columns:
            mcc_numeric = pd.to_numeric(rank_df['MCC Score'], errors='coerce')
            if not mcc_numeric.isna().all():
                metrics.append('MCC Score')
                rank_df['MCC Score'] = mcc_numeric
        ...
        for idx, row in results_df.iterrows():
            ...
            with st.expander(f"📊 {model_name} Analysis"):
                ...
                with col1:
                    st.write("**Strengths:**")
                    strengths = []
                    if 'Accuracy' in results_df.columns and row.get('Accuracy', 0) >= results_df['Accuracy'].quantile(0.75):
                        strengths.append("High Accuracy")
                    if 'Precision' in results_df.columns and row.get('Precision', 0) >= results_df['Precision'].quantile(0.75):
                        strengths.append("High Precision")
                    if 'Recall' in results_df.columns and row.get('Recall', 0) >= results_df['Recall'].quantile(0.75):
                        strengths.append("High Recall")
                    if 'F1 Score' in results_df.columns and row.get('F1 Score', 0) >= results_df['F1 Score'].quantile(0.75):
                        strengths.append("High F1 Score")
                    if 'AUC Score' in results_df.columns and pd.to_numeric(row.get('AUC Score', 0), errors='coerce') >= pd.to_numeric(results_df['AUC Score'], errors='coerce').quantile(0.75):
                        strengths.append("High AUC")
                    if 'MCC Score' in results_df.columns and pd.to_numeric(row.get('MCC Score', 0), errors='coerce') >= pd.to_numeric(results_df['MCC Score'], errors='coerce').quantile(0.75):
                        strengths.append("High MCC")
                ...
                with col2:
                    ...
                    weaknesses = []
                    if 'Accuracy' in results_df.columns and row.get('Accuracy', 0) <= results_df['Accuracy'].quantile(0.25):
                        weaknesses.append("Low Accuracy")
                    if 'Precision' in results_df.columns and row.get('Precision', 0) <= results_df['Precision'].quantile(0.25):
                        weaknesses.append("Low Precision")
                    if 'Recall' in results_df.columns and row.get('Recall', 0) <= results_df['Recall'].quantile(0.25):
                        weaknesses.append("Low Recall")
                    if 'F1 Score' in results_df.columns and row.get('F1 Score', 0) <= results_df['F1 Score'].quantile(0.25):
                        weaknesses.append("Low F1 Score")
                    if 'AUC Score' in results_df.columns and pd.to_numeric(row.get('AUC Score', 0), errors='coerce') <= pd.to_numeric(results_df['AUC Score'], errors='coerce').quantile(0.25):
                        weaknesses.append("Low AUC")
                    if 'MCC Score' in results_df.columns and pd.to_numeric(row.get('MCC Score', 0), errors='coerce') <= pd.to_numeric(results_df['MCC Score'], errors='coerce').quantile(0.25):
                        weaknesses.append("Low MCC")
                ...
                # Show numeric metric values and confusion matrix if available
                st.write("**Metrics:**")
                metric_items = []
                for m in ['Accuracy', 'Precision', 'Recall', 'F1 Score', 'AUC Score', 'MCC Score']:
                    if m in results_df.columns:
                        val = row.get(m, None)
                        try:
                            metric_items.append(f"- {m}: {float(val):.4f}")
                        except Exception:
                            metric_items.append(f"- {m}: {val}")
                if metric_items:
                    for it in metric_items:
                        st.write(it)

                # Confusion matrix display if present
                if 'Confusion_Matrix' in results_df.columns and pd.notna(row.get('Confusion_Matrix')):
                    try:
                        import ast
                        ...
                        cm_arr = np.array(cm)
                        fig_cm = go.Figure(data=go.Heatmap(z=cm_arr, x=['Pred Neg','Pred Pos'][:cm_arr.shape[1]], y=['True Neg','True Pos'][:cm_arr.shape[0]], colorscale='Blues'))
                        fig_cm.update_layout(title='Confusion Matrix')
                        st.plotly_chart(fig_cm, use_container_width=True)
                    except Exception:
                        st.write('Could not render confusion matrix for this model.')
